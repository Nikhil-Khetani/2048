    ...
    while episode<episodes:
        ...
        while type(output) == list:
            ...
            if new_game.getTerminal():
                ...
                reward, last_total = calcReward(next_state.squeeze(),last_total)
            next_state = torch.Tensor(np.array([[math.log2(i) if i!=0 else i for i in output]]))
            if new_game.getTurn()>3000:
                mem.push(state.unsqueeze(0),action,next_state,reward) # might need endgame in list
            ...
            if len(mem)>256:
                ...
                next_prediction_batch = model(next_state_batch).unsqueeze(0)
            ...
            print("Episode: {}/{}".format(episode+1, num_iters))
            if new_game.getTerminal():
                break
        if episode%30 == 0:
